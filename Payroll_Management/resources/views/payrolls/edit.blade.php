...
<form action="{{ route('payrolls.update', $payroll) }}" method="POST" class="max-w-md">
    ...
    <div class="mb-4">
        ...
        <select name="employee_id" id="employee_id" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" required>
            ...
            @foreach ($employees as $employee)
                <option value="{{ $employee->id }}" {{ $payroll->employee_id == $employee->id ? 'selected' : '' }}>{{ $employee->name }} ({{ $employee->position }})</option>
            @endforeach
        </select>
        @error('employee_id')
    ...
    <div class="mb-4">
        ...
        <input type="text" name="period" id="period" value="{{ old('period', $payroll->period) }}" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" required>
        @error('period')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="mb-4">
        ...
        <input type="number" step="0.01" name="amount" id="amount" value="{{ old('amount', $payroll->amount) }}" class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" min="0" required>
        @error('amount')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
